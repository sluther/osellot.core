<?php
class AuthNet extends Extension_Gateway_Osellot {
    ...
    public function checkoutOption() {
    	...
    	$tpl->display('devblocks:osellot.core::gateway/paypal/checkout_option.tpl');
    }
    
    public function checkout() {
    	$tpl = DevblocksPlatform::getTemplateService();
    	$tpl->display('devblocks:osellot.core::gateway/authnet/checkout.tpl');
    }
    ...
    public function configure() {
    	...
    	$tpl->display('devblocks:osellot.core::gateway/authnet/configure.tpl');
    }
				...
				if(null == ($result = DAO_Transaction::getWhere(sprintf("trans_id = %s", $trans[6]))))
					$id = DAO_Transaction::create(
									array(
										...
										DAO_Transaction::GATEWAY => 'wgm.osellot.gateway.cc.authnet',
...
//		var_dump($response);
	...
	public function encrypt($data) {
		$key = $this->generateKey(sha1($this->username), sha1('osellot'), 100000, 32);	
