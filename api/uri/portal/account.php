<?php
class AccountPortal_OsellotController extends Extension_Portal_Osellot_Controller {	
	public function writeResponse(DevblocksHttpResponse $response) {
		...
		switch($section) {
			case 'history':
				...
				$tpl->display('devblocks:osellot.core::portal/account/history.tpl');
				break;
			case 'invoice':
				$invoice_id = array_shift($stack);
				$invoice = DAO_Invoice::get($invoice_id);
				...
				foreach($attributes as $attribute) {
					...
					if(0 != ($pos = strpos($attribute->name, '.'))) {
						$prefix = substr($attribute->name, 0, $pos);
						$key = substr($attribute->name, $pos+1);
						$invoice->attributes[$prefix][$key] = $attribute->value;
					} else {
						$invoice->attributes[$attribute->name] = $attribute->value;
					}
				}
				$plugin = DevblocksPlatform::getExtension($invoice->getAttribute('checkout_plugin'), true);
				$tpl->assign('plugin', $plugin);
				$tpl->assign('invoice', $invoice);
				$tpl->display('devblocks:osellot.core::portal/account/invoice.tpl');
				break;
			case 'edit':
				$tpl->display('devblocks:osellot.core::portal/account/edit.tpl');
				break;
			default:
				$tpl->display('devblocks:osellot.core::portal/account/index.tpl');
				break;
		}
	}
