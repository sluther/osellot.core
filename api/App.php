...
class GenerateInvoicesCron extends CerberusCronPageExtension {
	function run() {
		$logger = DevblocksPlatform::getConsoleLog();

		$logger->info("[The Good Food Box] Starting Recurring Invoice Generation Task");
		$runtime = time();
		$date = $this->getParam('date');

		$ship_date = DevblocksPlatform::getPluginSetting('wgm.osellot', 'recurring', 0);
		// convert to int
		$date = strtotime($date);
		// Should we shift the time ahead 2 weeks?
		if($date <= time()) {
			$ship_date = strtotime('next wednesday + 1 week', $date);
			$this->setParam('date', date('F d, Y', $ship_date));
		}
		if($date >= $ship_date) {
			$logger->info(sprintf("[The Good Food Box] Updated Ship Date to %s", date('F d, Y', $date)));
			DevblocksPlatform::setPluginSetting('bitpiston.tgfb', 'ship_date', $ship_date);
		}

		@set_time_limit(1200); // 20m

		$logger->info("[The Good Food Box] Total Runtime: ".number_format((microtime(true)-$runtime)*1000,2)." ms");
	}
