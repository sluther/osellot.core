 ...
 */
class HummingbirdPage extends CerberusPageExtension {
	private $_TPL_PATH = '';
	private $cerb5;
	private $base_url;
	...
	function render() {
		...
		if(null == ($selected_tab = @$response->path[1])) {
			$selected_tab = $visit->get(Extension_Tab_Billing_Hummingbird::POINT, '');
		}
		...
		$tab_manifests = DevblocksPlatform::getExtensions('tab.billing.hummingbird');
		$tpl->assign('tab_manifests', $tab_manifests);
		$tpl->display('devblocks:hummingbird.core::billing/index.tpl');
...
//		$authnet->processTransaction($transaction);
	...
	function showTabAction() {
		...
		if(null != ($tab_mft = DevblocksPlatform::getExtension($ext_id)) 
			...
			&& $inst instanceof Extension_Tab_Billing_Hummingbird) {
				$visit->set(Extension_Tab_Billing_Hummingbird::POINT, $inst->manifest->params['uri']);
				$inst->showTab();
		}
	}
	...
	*/
	...
	public function createAction() {		
		...
		$gateway_plugins = DevblocksPlatform::getExtensions('wgm.hummingbird.gateway.cc');

		$authnet = DevblocksPlatform::getExtension('wgm.hummingbird.gateway.cc.authnet', true);
...
class SetupSection_Hummingbird extends Extension_PageSection {
	const ID = 'setup.hummingbird.core';
...
// 		);
		...
		$visit = CerberusApplication::getVisit();
		$visit->set(ChConfigurationPage::ID, 'hummingbird');
		
		$tab_manifests = DevblocksPlatform::getExtensions('tab.setup.hummingbird.core');
		...
		$tpl->assign('tab_manifests', $tab_manifests);
		$tpl->display('devblocks:hummingbird.core::configuration/section/billing/index.tpl');
	}
	...
	function showTabAction() {
		...
		if(null != ($tab_mft = DevblocksPlatform::getExtension($ext_id)) 
			&& null != ($inst = $tab_mft->createInstance()) 
			&& $inst instanceof Extension_HummingbirdSetupTab) {
...
abstract class Extension_Portal_Hummingbird_Controller extends DevblocksExtension implements DevblocksHttpRequestHandler {
	private $portal = '';
...
abstract class Extension_Agency_Portal_Hummingbird_Controller extends DevblocksExtension implements DevblocksHttpRequestHandler {
...
abstract class Extension_HbLoginAuthenticator extends DevblocksExtension {
...
class UmHbApp extends Extension_UsermeetTool {
    ...
    private function _getModules() {
    	...
    	if(null == $modules) {
			...
			foreach($visible_modules as $module_id => $visibility) {
				...
				$module = DevblocksPlatform::getExtension($module_id,true,true); /* @var $module Extension_UmHbController */
				
				if(empty($module) || !$module instanceof Extension_Portal_Hummingbird_Controller)
					continue;
	...
	public function writeResponse(DevblocksHttpResponse $response) {
        ...
        $login_extensions_enabled = UmHbApp::getLoginExtensionsEnabled(ChPortalHelper::getCode());
		...
		switch($module_uri) {
			case 'rss':
				$controller = new UmHbRssController(null);
				$controller->handleRequest(new DevblocksHttpRequest($stack));
				break;
			...
			case 'captcha':
		        ...
		        $umsession->setProperty(UmHbApp::SESSION_CAPTCHA, $phrase);
			...
			case 'captcha.check':
				...
				$captcha = $umsession->getProperty(UmHbApp::SESSION_CAPTCHA, '');
	    	...
	    	default:
				...
				$tpl->assign('module_response', new DevblocksHttpResponse($stack));
   				...
   				$tpl->display('devblocks:hummingbird.core:portal_'.ChPortalHelper::getCode() . ":portal/index.tpl");
		    	break;
		}
	}
    ...
    public function configure(Model_CommunityTool $instance) {
		...
		$all_modules = DevblocksPlatform::getExtensions('portal.controller.hummingbird', true, true);
		$modules = array();
        ...
        $tpl->display("devblocks:hummingbird.core::portal/hb/config/index.tpl");
    }
    ...
    public function saveConfiguration(Model_CommunityTool $instance) {
		...
		$modules = DevblocksPlatform::getExtensions('portal.controller.hummingbird',true,true);
		foreach($modules as $module) { /* @var $module Extension_Portal_Hummingbird_Controller */
			// Only save enabled
...
class UmHbLoginAuthenticator extends Extension_HbLoginAuthenticator {
	function writeResponse(DevblocksHttpResponse $response) {
		...
		switch($module) {
			case 'agency':
				$tpl->display("devblocks:hummingbird.core:portal_".ChPortalHelper::getCode().":portal/agency/login/login.tpl");
				break;			
			case 'register':
				if(isset($section) && 0==strcasecmp('confirmed', $section)) {
					$tpl->display("devblocks:hummingbird.core:portal_".ChPortalHelper::getCode().":portal/login/registered.tpl");
				} else {
					$tpl->assign('title', 'Sign-up with the Good Food Box to order online');
					$tpl->display("devblocks:hummingbird.core:portal_".ChPortalHelper::getCode().":portal/login/register.tpl");
				}
				break;
			case 'forgot':
				if(isset($section) && 0==strcasecmp('confirm', $section)) {
					$tpl->display("devblocks:hummingbird.core:portal_".ChPortalHelper::getCode().":portal/login/forgot_confirm.tpl");
				} else {
					$tpl->display("devblocks:hummingbird.core:portal_".ChPortalHelper::getCode().":portal/login/forgot.tpl");
				}
				break;
			default:
				$tpl->display("devblocks:hummingbird.core:portal_".ChPortalHelper::getCode().":portal/login/login.tpl");
			break;
		}
	}
...
abstract class Extension_Tab_Billing_Hummingbird extends DevblocksExtension {
	const POINT = 'tab.billing.hummingbird'; 
	
	public function showTab() {}
}

abstract class Extension_Gateway_Hummingbird extends DevblocksExtension {
...
class PageMenu_Setup_Hummingbird extends Extension_PageMenu {
	
	public function render() {
		$tpl = DevblocksPlatform::getTemplateService();
		$tpl->display('devblocks:hummingbird.core::configuration/section/billing/menu.tpl');
	}
};