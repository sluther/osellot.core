<?php
class CollatingTab_Billing_Hummingbird extends Extension_Tab_Billing_Hummingbird {
...
// 		$tpl->assign('items', $items);
		...
		$orders = DAO_Invoice::getByDateRangeAndStatus($startdate, $enddate, $statuses);
		...
		$tpl->display('devblocks:hummingbird.core::billing/tabs/collating.tpl');		
	}
	...
	public function calculateAction() {		
		...
		$excluded = array('id', 'item', 'source', 'origin', 'unit', 'weighed', 'casesneeded');
...
// 			multiply units * total boxes then add to casesneeded
		...
		$row['casesrounded'] = ceil($row['casesneeded']);
		$row['remainder'] = ($row['casesrounded'] - $row['casesneeded']) * $row['unitspercase'];
		
		if($row['remainder'] < (0.03 * $row['casesrounded'] * $row['unitspercase']))
			$row['guidance'] = 'tight';
		elseif($row['remainder'] > (0.1 * $row['casesrounded'] * $row['unitspercase']))
			$row['guidance'] = 'loose';
		else
			$row['guidance'] = 'ok';
