...
if(!isset($tables['agency'])) {
	$sql = sprintf("
		CREATE TABLE IF NOT EXISTS agency (
			...
			address_line2 VARCHAR(255) DEFAULT '' NOT NULL,
			address_city VARCHAR(255) DEFAULT '' NOT NULL,
			address_province VARCHAR(255) DEFAULT '' NOT NULL,
			address_postal VARCHAR(255) DEFAULT '' NOT NULL,
			position INT UNSIGNED DEFAULT 0 NOT NULL,
			is_agency INT UNSIGNED DEFAULT 0 NOT NULL,
			PRIMARY KEY (id)
		) ENGINE=%s;
	", APP_DB_ENGINE);
	$db->Execute($sql);
...
if(!isset($columns['is_agency']))
	$db->Execute("ALTER TABLE contact_person ADD COLUMN is_agency INT UNSIGNED NOT NULL DEFAULT 0");

$gateway_plugins = DevblocksPlatform::getExtensions('cc.gateway.hummingbird.core');
